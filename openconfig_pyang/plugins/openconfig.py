...
INSTANTIATED_DATA_KEYWORDS = ["leaf", "leaf-list", "container", "list", "choice"]
...
class ExternalValidationRules(object):
  ...
  """
    ...
    required_substatements = {
        "module": (
            ("contact", "organization", "description", "revision"),
            "RFC 6087: 4.7",
        ),
        "submodule": (
            ("contact", "organization", "description", "revision"),
    ...
    def setup_ctx(self, ctx):
        ...
        if not ctx.opts.openconfig_only:
            ...
            statements.add_validation_var(
                ...
                in ExternalValidationRules.required_substatements,
            )
            ...
            statements.add_validation_fun(
                "grammar", ["$chk_default"], lambda ctx, s: lint.v_chk_default(ctx, s)
            )
            statements.add_validation_fun(
                "grammar",
                ["$chk_required"],
            ...
            statements.add_validation_fun(
                "strict", ["include"], lambda ctx, s: lint.v_chk_include(ctx, s)
            )

            # Register the default linter error codes
            error.add_error_code(
                "LINT_EXPLICIT_DEFAULT",
                ErrorLevel.WARNING,
                "RFC 6087: 4.3: " 'statement "%s" is given with its default value "%s"',
            )
            error.add_error_code(
            ...
            error.add_error_code(
                "LINT_BAD_MODULENAME_PREFIX_1",
                ErrorLevel.WARNING,
                "RFC 6087: 4.1: " "the module name should start with the string %s",
            )
            error.add_error_code(
            ...
            error.add_error_code(
                "LINT_TOP_MANDATORY",
                ErrorLevel.MINOR,
                "RFC 6087: 4.9: " "top-level node %s must not be mandatory",
            )
            error.add_error_code(
                "LONG_IDENTIFIER",
                ErrorLevel.MINOR,
        ...
        statements.add_validation_fun("preinit", ["*"], OCLintStages.preinitialisation)
        ...
        error.add_error_code(
            "OC_ENUM_UNDERSCORES",
            ErrorLevel.MAJOR,
            'enum value "%s" should be of the form ' "UPPERCASE_WITH_UNDERSCORES: %s",
        )
        ...
        error.add_error_code(
            "OC_LIST_NO_ENCLOSING_CONTAINER",
            ErrorLevel.MAJOR,
        ...
        error.add_error_code(
            "OC_GROUPING_NAMING_NONSTANDARD",
            ErrorLevel.WARNING,
...
class OCLintStages(object):
  ...
  """
    ...
    """
        if stmt.i_module is not None and stmt.i_module.arg in [
            "iana-if-type",
            "ietf-interfaces",
        ]:
            return
    ...
    """
        ...
        validmap = {
            ...
            "identity": [OCLintFunctions.check_identity_style],
            "module": [
                OCLintFunctions.check_versioning,
                OCLintFunctions.check_top_level_data_definitions,
                OCLintFunctions.check_standard_groupings,
            ],
            "augment": [OCLintFunctions.check_relative_paths],
            "path": [OCLintFunctions.check_relative_paths],
            "typedef": [OCLintFunctions.check_typedef_style],
        }
    ...
    """
        validmap = {
            "LEAVES": [OCLintFunctions.check_opstate],
            "list": [
                OCLintFunctions.check_list_enclosing_container,
                OCLintFunctions.check_leaf_mirroring,
            ],
            "container": [OCLintFunctions.check_leaf_mirroring],
        }
...
class OCLintFunctions(object):
    ...
    """
        if re.match(r"[a-z0-9]+\-.*", mod.lower()):
            ...
            if modname_parts[0] in ["ietf", "iana"]:
    ...
    """
        ...
        for substmt in stmt.substmts:
            ...
            # modules.
            if (
                isinstance(substmt.keyword, tuple)
                and substmt.keyword[1] == "openconfig-version"
            ):
                version = substmt
    ...
    """
        ...
        for enum in elemtype.search("enum"):
            if re.match(r"[a-z]", enum.arg):
                err_add(
                    ctx.errors, stmt.pos, "OC_ENUM_CASE", (enum.arg, enum.arg.upper())
                )
    ...
    """
        ...
        if re.match(r"^[a-z]", stmt.arg):
            err_add(
                ctx.errors, stmt.pos, "OC_IDENTITY_CASE", (stmt.arg, stmt.arg.upper())
            )
    ...
    """
        ...
        if stmt.parent.keyword != "container":
            err_add(
                ctx.errors, stmt.parent.pos, "OC_LIST_NO_ENCLOSING_CONTAINER", stmt.arg
            )
        ...
        for ch in grandparent.i_children:
            if ch.keyword == "container" and ch.arg != stmt.parent.arg:
                if (
                    len(ch.i_children) == 1
                    and ch.i_children[0].arg == stmt.arg
    ...
    """
        if stmt.keyword == "choice":
            ...
            err_add(ctx.errors, stmt.pos, "OC_STYLE_AVOID_PRESENCE", (stmt.parent.arg))
    ...
    """
        ...
        if namespace == mod_prefix and abspath:
            ...
            if not is_typedef:
                err_add(
                    ctx.errors, stmt.pos, "OC_RELATIVE_PATH", (stmt.keyword, stmt.arg)
                )
    ...
    """
        ...
        if not found:
            err_add(
                ctx.errors, stmt.pos, "OC_MISSING_STANDARD_GROUPING", (stmt.arg, "-top")
            )
