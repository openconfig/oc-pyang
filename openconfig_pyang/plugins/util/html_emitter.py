...
class HTMLEmitter(DocEmitter):
    ...
    ModuleDoc object"""
        ...
        mod_div += ht.h1(
            mod.module_name,
            {"class": "module-name", "id": ("mod-" + ht.gen_html_id(mod.module_name))},
            2,
            True,
        )
        ...
        if "version" in mod.module.attrs:
            mod_div += ht.h4(
                "openconfig-version: " + mod.module.attrs["version"],
                {"class": "module-header"},
                2,
                True,
            )
        ...
        if len(mod.typedefs) > 0:
            ...
            types_div += ht.h3(
                "Defined types",
                {
                    "class": "module-types-header",
                    "id": mod.module_name + "-defined-types",
                },
                2,
                True,
            )
            ...
            for (typename, td) in mod.typedefs.items():
                types_div += ht.h4(
                    typename,
                    {
                        "class": "module-type-name",
                        "id": "type-" + ht.gen_html_id(typename),
                    },
                    2,
                    True,
                )
                types_div += ht.para(
                    ht.add_tag(
                        "span",
                        "description:" + ht.br(newline=True),
                        {"class": "module-type-text-label"},
                    )
                    + td.attrs["desc"],
                    {"class": "module-type-text"},
                    2,
                    True,
                )
                ...
                for prop in YangDocDefs.type_leaf_properties:
                    if prop in td.attrs:
                        types_div += ht.para(
                            ht.add_tag(
                                "span", prop, {"class": "module-type-text-label"}
                            )
                            + ": "
                            + td.attrs[prop],
                            {"class": "module-type-text"},
        ...
        if len(mod.identities) > 0:
            ...
            idents_div += ht.h3(
                "Identities",
                {"class": "module-types-header", "id": mod.module_name + "-identities"},
                2,
                True,
            )
            ...
            for base_id in mod.base_identities:
                idents_div += ht.h4(
                    ...
                    {
                        "class": "module-type-name",
                        "id": "ident-" + ht.gen_html_id(base_id),
                    },
                    2,
                    True,
                )
                ...
                for (idname, id) in derived.items():
                    idents_div += ht.h4(
                        ...
                        {
                            "class": "module-type-name",
                            "id": "ident-" + ht.gen_html_id(idname),
                        },
                        2,
                        True,
                    )
                    idents_div += ht.para(
                        ht.add_tag(
                            "span", "description:", {"class": "module-type-text-label"}
                        )
                        + ht.br(newline=True)
                        + id.attrs["desc"],
                        {"class": "module-type-text"},
    ...
    object"""
        ...
        s_div += ht.h4(
            statement_name,
            {"class": "statement-name", "id": statement.attrs["id"]},
            level,
            True,
        )
        ...
        if "desc" in statement.attrs:
            s_div += ht.para(
                ht.add_tag("span", "description", {"class": "statement-info-label"})
                + ":<br />"
                + statement.attrs["desc"],
                {"class": "statement-info-text"},
                level,
                True,
            )
        s_div += ht.close_tag(newline=True)
        ...
        s_div += ht.para(
            ht.add_tag("span", "nodetype", {"class": "statement-info-label"})
            + ": "
            + keyword,
            {"class": "statement-info-text"},
            level,
        ...
        if statement.attrs["is_list"]:
            ...
            for key in statement.attrs["keys"]:
                list_keys += (
                    " [" + ht.add_tag("a", key[0], {"href": "#" + key[1]}) + "]"
                )
            s_div += ht.para(
                ht.add_tag("span", "list keys", {"class": "statement-info-label"})
                + ": "
                + list_keys,
                {"class": "statement-info-text"},
                level,
        ...
        for prop in YangDocDefs.type_leaf_properties:
            if prop in statement.attrs:
                s_div += ht.para(
                    ht.add_tag("span", prop, {"class": "statement-info-label"})
                    + ": "
                    + statement.attrs[prop],
                    {"class": "statement-info-text"},
                    level,
                    True,
                )
    ...
    or single section if specified"""
        ...
        for module_name in self.moduledocs:
            ...
            if "data" not in self.moduledocs[module_name]:
                ...
                hdr = ht.h3(
                    "Data elements",
                    {"class": "module-types-header", "id": module_name + "-data"},
                    2,
                    True,
                )
                self.moduledocs[module_name]["data"] = (
                    hdr + self.moduledocs[module_name]["data"]
                )
            ...
            if section is not None:
                ...
                docs.append(
                    self.moduledocs[module_name]["module"]
...
def gen_type_info(typedoc, level=1):
  ...
  types."""
    ...
    s += ht.para(
        ht.add_tag("span", "type", {"class": "statement-info-label"}) + ": " + typename,
        {"class": "statement-info-text"},
        level,
        True,
    )
...
def populate_template(title, docs, navs, nav_ids):
    ...
    return template.render(
        {"title": title, "htmldocs": docs, "menus": navs, "menu_ids": nav_ids}
    )
...
def gen_nav_tree(emitter, root_mod, level=0):
  ...
  module.  root_mod is a top-level ModuleDoc object"""
    ...
    nav += '<li><a class="menu-module-name" href="%s">%s</a></li>\n' % (
        "#mod-" + ht.gen_html_id(root_mod.module_name),
        root_mod.module_name,
    )

    # generate links for types and identities
    if len(root_mod.typedefs) > 0:
        nav += '<li><a href="%s">%s</a>\n' % (
            "#" + ht.gen_html_id(root_mod.module_name) + "-defined-types",
            "Defined types",
        )
    ...
    if len(root_mod.identities) > 0:
        nav += '<li><a href="%s">%s</a>\n' % (
            "#" + ht.gen_html_id(root_mod.module_name) + "-identities",
            "Identities",
        )
        nav += " <ul>\n"
        for base_id in root_mod.base_identities:
            derived = {
                key: value
                for key, value in root_mod.identities.items()
                if value.attrs["base"] == base_id
            }
            nav += '  <li><a href="%s">%s</a>\n' % (
                "#ident-" + ht.gen_html_id(base_id),
                base_id,
            )
            nav += "  <ul>\n"
            for idname in derived.keys():
                nav += '    <li><a href="%s">%s</a></li>\n' % (
                    "#ident-" + ht.gen_html_id(idname),
                    idname,
                )
            nav += "  </ul>\n"
            nav += "  </li>\n"
    ...
    emitter.moduledocs[root_mod.module_name]["navid"] = "tree-" + ht.gen_html_id(
        root_mod.module_name
    )
...
def gen_nav(node, root_mod, level=0):
    ...
    if len(node.children) > 0:
        ...
        nav += (
            " " * current_level + " <li>"
            '<a href="#' + node.attrs["id"] + '">' + node.name + "</a>\n"
        )
